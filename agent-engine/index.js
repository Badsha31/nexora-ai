...
async function executeAction(project,a){
 ...
 case 'build':{const result=await build(project,String(a.command||'npm run build'));return {type:a.type,success:true,result:{code:result.code,stdout:result.stdout?.slice(0,16000),stderr:result.stderr?.slice(0,16000)}};}
 case 'test':{const result=await runTests(project,String(a.command||'npm test'));return {type:a.type,success:true,result:{code:result.code,stdout:result.stdout?.slice(0,16000),stderr:result.stderr?.slice(0,16000)}};}
