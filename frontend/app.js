...
async function buildProject(){try{const p=state.currentProject;if(p.type==='android'){const x=await api('/api/projects/'+p.id+'/android-build',{method:'POST',body:JSON.stringify({variant:'debug'})});add('ai','Android build result:\n'+JSON.stringify(x,null,2));if(x.apks?.length){const m=$('#messages');m.insertAdjacentHTML('beforeend','<div class="apk-links">'+x.apks.map(a=>'<a href="/api/projects/'+p.id+'/apk?path='+encodeURIComponent(a.path)+'">Download '+escapeHtml(a.path)+'</a>').join('')+'</div>');}}else{const x=await api('/api/projects/'+p.id+'/build',{method:'POST',body:JSON.stringify({})});add('ai','Build result:\n'+JSON.stringify(x,null,2));}}catch(e){add('ai','Build error: '+e.message)}}
