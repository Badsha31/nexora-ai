...
export async function health(){
  const c=modelConfig();
  try{
    const r=await fetch(endpoint(c,'/models'),{signal:AbortSignal.timeout(10000),headers:c.apiKey?{Authorization:`Bearer ${c.apiKey}`}:{}});
    let body=null;try{body=await r.json()}catch{}
    return {available:r.ok,status:r.status,model:c.model,endpoint:c.baseUrl,serverModels:body?.data||[],config:{...c,apiKey:c.apiKey?'configured':'not required'}};
  }catch(e){
    return {available:false,error:e.message,model:c.model,endpoint:c.baseUrl,config:{...c,apiKey:c.apiKey?'configured':'not required'}};
  }
}
export async function chat(messages,options={}){
  const c=modelConfig();
  try{
    const payload={model:c.model,messages,temperature:options.temperature??0.15,max_tokens:options.maxTokens??32768,stream:false};
    ...
    const r=await fetch(endpoint(c,'/chat/completions'),{method:'POST',headers,body:JSON.stringify(payload),signal:AbortSignal.timeout(options.timeoutMs??240000)});
    if(!r.ok){
    ...
    const j=await r.json();const message=j.choices?.[0]?.message;
  ...
  }catch(e){if(e.code)throw e;throw err('MODEL_UNAVAILABLE',`Nexora Coding Model unavailable: ${e.message}`,503);}
}
